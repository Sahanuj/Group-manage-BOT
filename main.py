...
async def panel_cmd(message: types.Message):
    ...
    if message.chat.type != "private":
        return await message.reply("Use /panel in DM with me.")
    await message.reply("🔐 <b>Group Guardian Panel</b>", reply_markup=get_main_panel())

@dp.callback_query(lambda c: c.data == "back_main")
async def back_main(callback: CallbackQuery):
    await callback.message.edit_text("🔐 <b>Group Guardian Panel</b>", reply_markup=get_main_panel())
...
async def get_buttons(message: types.Message, state: FSMContext):
    ...
    await group.save()
    await message.reply("✅ Saved!", reply_markup=get_main_panel())
    await state.clear()
